...
pack_robot_path = "xxxxxx"                 # 打包后ipa存储目录 请指向自动打包脚本所在目录
exportOptionsPlist = "xxxxxx/ipaAutoPack"  # Archive plist路径
...
if isXcodeAutoSign == "n":
    ...
    if mobileprovision_uuid.find("xxxxxx") != -1 \
    or signing_certificate.find("xxxxxx") != -1:
    ...
    if os.path.exists("%s/%s" % (local_path_filename,ipa_filename)):
        ...
        if env == "dev":
            if fir_api_token.find("xxxxxx") == -1:
                ...
                ret = os.system("fir publish '%s' --token='%s'" % (ipafilePath, fir_api_token))
            elif pgyer_uKey.find("xxxxxx") == -1:
                ret = os.system('curl -F "file=@%s" -F "uKey=%s" -F "_api_key=%s" -F "installType=%s" -F "password=%s" http://www.pgyer.com/apiv1/app/upload' % (ipafilePath, pgyer_uKey, pgyer_apiKey, pgyer_installType, pgyer_password))
        elif env == "dis":
            if appstore_login_name.find("xxxxxx") == -1:
                altoolPath = "/Applications/Xcode.app/Contents/Applications/Application\ Loader.app/Contents/Frameworks/ITunesSoftwareService.framework/Versions/A/Support/altool"
                ret = os.system('%s --validate-app -f %s -u %s -p %s -t ios --output-format xml' % (altoolPath,ipafilePath,appstore_login_name,appstore_login_pwd))
                ret = os.system('%s --upload-app -f %s -u %s -p %s -t ios --output-format xml' % (
...
def send_mail():
    if from_addr != "":
        appUrl = ""
        if fir_api_token.find("xxxxxx") == -1:
            appUrl = download_address
        elif pgyer_uKey.find("xxxxxx") == -1:
            appUrl = pgyer_appQRCodeURL
        msg = MIMEText(app_name + " iOS客户端已经打包完毕，请前往 " + appUrl + " 下载测试！如有问题，请联系iOS相关人员或者直接将问题提至Teambition，我们会及时解决，谢谢!", 'plain', 'utf-8')
